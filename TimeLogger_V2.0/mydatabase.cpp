...
bool MyDatabase::signUpCheckQuery(QString user, QString password, QString name, QString surname, bool isAdmin)
{
    bool isOkay = false;
    int adminChecked=0;
    if(isAdmin)
    ...
    else
    {
        ...
        if(!checker)
        {
            ...
            queryFillInTable.prepare("insert into workers values ('"+user+"','"+name+"','"+surname+"','"+cryptedPassword+"',:adminChecked);");
    ...
    else
    {
        ...
        while(queryAuthentication.next())
        {
            if((queryAuthentication.value(0) == user) && (queryAuthentication.value(1) == cryptedPassword))
            {
                ...
                MyDatabase::instance()->setIsAdmin(queryAuthentication.value(2).toInt());
                isOkay = true;
                break;
            }
            else {
...
bool MyDatabase::getAllUserReportsQuery(QString user,QString projectName)
{
    bool isOkay = false;
    //Query that selects all the information from the report table that is needed to be displayed in the table
    QSqlQuery loadReport(m_myDatabase);
...
bool MyDatabase::getAllUserReportsCountQuery(QString user, QString projectName)
{
    bool isOkay = false;
    //Query that counts the number of reports this week from the user so it can set the row count of the table
    QSqlQuery getSize(m_myDatabase);
    if(projectName == "All Projects")
    {
        getSize.prepare("select count(description) from reports where wuser=='"+user+"'");
    }
    else
        getSize.prepare("select count(description) from reports where wuser=='"+user+"' AND pname=='"+projectName+"'");
    m_reportsCount = 0;
    if(!getSize.exec()){
        qDebug()<<"ERROR : couldn't count the reports";
    }
...
bool MyDatabase::getAllUserProjects(QString user)
{
    ...
    {
        emit setCheckBoxProjects("All Projects");
        while(getProjects.next())
        {
            QString text;
            text = getProjects.value(0).toString();
...
int MyDatabase::getIsAdmin() const
{
    return m_isAdmin;
}

void MyDatabase::setIsAdmin(int isAdmin)
{
    m_isAdmin = isAdmin;
}
