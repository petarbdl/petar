...
void Widget::browseFiles()
{
    ...
    name = QFileDialog::getOpenFileName(this,tr("Chose File"),downloadsFolder,tr("*.mp4, *.wmv, *.avi"));
...
void Widget::pushButtonFind_clicked()
{
    ...
    if(QFileInfo::exists(newFile.filePath()))
    {
        if(newFile.isDir())
        {
            name = QFileDialog::getOpenFileName(this,tr("Choose File"),filePath,tr("*.mp4, *.wmv, *.avi"));
            QStringList tmp = name.split("/");
            name = tmp.at(tmp.count()-1);
            if(name != "")
            {
