...
void GetUserReport::on_tableWidget_cellEntered(int row, int column)
{
    if(column == 2)
    {
        QString description = ui->tableWidget->item(row,column)->text();
        QPoint point;
        point = ui->tableWidget->visualItemRect(ui->tableWidget->item(row,column)).center();
        QToolTip::showText(ui->tableWidget->mapToGlobal(QPoint(point)),description);
    }
}
