...
void TimeStatus::keyPressEvent(QKeyEvent *event)
{
...
void TimeStatus::mouseDoubleClickEvent(QMouseEvent *event)
{
    Q_UNUSED(event)
...
//LogoutButton
void TimeStatus::on_buttonLogout_clicked()
{
    m_exitDialog.setWindowTitle("Logout...");
    m_exitDialog.setValues(MyDatabase::instance()->getCurrentUser());
    m_exitDialog.exec();
}

void TimeStatus::onLogoutDialogLogoutPressed()
{
    emit logoutPressed();
...
void TimeStatus::onLogoutDialogQuitPressed()
{
    qDebug()<<"Quit app...";
    m_exitDialog.close();
    close();
}
//Show ToolTip information when cell is hovered
void TimeStatus::on_tableWidgetReportDetails_cellEntered(int row, int column)
{
    if(column == 2)
    {
