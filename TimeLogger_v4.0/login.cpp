    ...
    ui(new Ui::Login)
{
    ...
    //When Enter is pressed, try to Login the user
    connect(ui->lineEditPassLog, SIGNAL(returnPressed()),ui->buttonLogInCheck,SIGNAL(clicked()));
    connect(ui->lineEditUserLog, SIGNAL(returnPressed()),ui->buttonLogInCheck,SIGNAL(clicked()));
}
