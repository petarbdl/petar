...
void DBConnection::reorderItems(int listIndex, bool done, QString name, int position)
{
    QSqlQuery query;
    query.prepare("INSERT INTO ItemTable (ItemId, ItemName, ItemDone, ItemPosition) VALUES(:itemId, :itemName, :itemDone, :itemPosition)");
...
void DBConnection::deleteListInDB(int &listIndex)
{
    ...
    //Delete list from database
    QSqlQuery query2;
    query2.prepare("DELETE FROM ItemTable WHERE ItemId = :index");
    query2.bindValue(":index", listIndex);
    query2.exec();
}
