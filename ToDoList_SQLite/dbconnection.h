...
public:
    DBConnection();
    //This function is called when user is finished Reordering, it enters one List item at a time
    void reorderListInDB(int &listIndex, const QString name, const bool done, const int position);
    ...
    //Inserts Item in database after reordering
